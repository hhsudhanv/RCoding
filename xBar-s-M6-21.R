...
controlChart = "x-bar, s" # variable, n>=10
...
# Chart titles
sTitle = "s for Compressive Strength (psi)" # s chart title
...
B3 = 0     # s chart LCL
B4 = 2.089 # s chart UCL
c4 = 0.940 # unbiased estinate of standard deviation when using s chart
...
# s for each row = sample standard deviation
sData = data
s = apply(data[,1:n], 1, sd) # Montgomery 6.3.1
print(s)

# s-bar is the mean of the sample standard deviations
sBar = mean(s[1:phaseIEnd]) # Montgomery 6.3.1
print(sBar)

# Where the s data's Phase I ends and Phase II starts
sPhaseIEnd = phaseIEnd
sPhaseIIStart = phaseIIStart

# Unbiased estimate of the standard deviation.
sigmaHat = sBar / c4 # Montgomery 6.3.1
print(sigmaHat)
...
# s chart's control limits.  We usually don't worry about warning and
...
# The s chart is also a good way to look at this.
...
# s is not assumed to be normal so we don't do normality checks.
# s-bar is assumed to be normal (applying Central Limit Theorem), 
# but it is a single statistic so we cannot test for normality 
# of its distribution.
...
# Draw the s chart.
sChart = qcc(sData[1:sPhaseIEnd,1:n], 
             type="S",
             title=sTitle,
             xlab=sampleDescription,
             ylab=qualityCharacteristic)
...
# s chart.  Phase I centerline and control limits are used.
sChart2 = qcc(sData[1:sPhaseIEnd,], 
              type="S", 
              newdata=sData[sPhaseIIStart:nrow(sData),],
              title=sTitle,
              xlab=sampleDescription,
              ylab=qualityCharacteristic)
...
# If any of the points are out of control, it's helpful to get their
# actual values.
print(s[sPhaseIIStart:nrow(sData)])
